...
К счастью, в Node.js встроен помощник на такие случаи — профайлер. Точнее, он встроен в V8 (что это такое, можно почитать [здесь](https://sptmru.github.io/ru/2023/asynchrony-in-js/) или даже [здесь](https://v8.dev)).
Профайлер V8 — это такая штука, которая делит стек выполняющейся программы (о стеке можно почитать [в моей статье об управлении памятью в JS](https://sptmru.github.io/ru/2023/memory-management-in-js/)) на фрагменты через равные промежутки времени.
...
К счастью, мы понимаем, что эта функция работает синхронно и блокирует event loop (если не понимаем, о чем речь, читаем вот эту [статью об асинхронности](https://sptmru.github.io/ru/2023/asynchrony-in-js/)).
