...
В отличие от модели "запрос-ответ", используемой в HTTP/1.1, в HTTP/2 используются мультиплексированные потоки — данные в обе стороны передаются в рамках одного TCP-соединения ([статья о TCP тут:](https://sptm.space/2023/tcp-udp)) в бинарном формате. Такой подход дает кучу преимуществ:
